theory HOLCSP_turtlebot_makeplan_ddl_ag_true
	imports "RC_HOL-CSP.RoboChart_Library" HOL.Real 
begin

subsection \<open> Model \<close>
(*
In this version, Trans model is correct,same as the one in "HOLCSP_turtlebot_makeplan_ddlf.thy" 
but the inv is set as default True, it is no longer deadlock free, so a counterexample is identified.
*)
...
(*counterexample found for inv=True*)
lemma Trans_MakePlan_Adaptation_Plan_ag_true_counterexample_auto:
  "deadlock_free\<^sub>S\<^sub>K\<^sub>I\<^sub>P\<^sub>S (\<sqinter> n \<in> UNIV. Trans_MakePlan_Adaptation_Plan\<cdot>n)"
  apply (find_counterexample P_def: Trans_MakePlan_Adaptation_Plan.simps)
  quickcheck
(*
Quickcheck found a counterexample:
  st_var__ = NID_CalculateRotations_MakePlan_Adaptation_Plan
  a__ = real_of_rat (Fract 0 1)
  b__ = real_of_rat (Fract (- 1) 1)
*)
  oops
